...
import type { attractionFetchData } from "@/type/attraction";
...
const Header = () => {
    ...
    const [categoryBlog, setCategoryBlog] = useState<attractionFetchData[] | null>([]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const result = await actions.categoryBlog();
                setCategoryBlog(result);
            } catch (error) {
                console.error("Error fetching categoryBlog:", error);
            }
        };
    ...
    return (
        <>
            <svg className="hidden">
                ...
                <symbol id="logo" viewBox="0 0 41 40" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <nav className='flex gap-x-3 lg:gap-x-9 h-14 items-center justify-between w-full'>
                        ...
                        <div className="relative flex justify-start w-full items-center text-white drop-shadow-md">
                            ...
                            <div className="flex items-center justify-between w-full">
                                ...
                                <div className="flex items-center gap-x-5 text-gray-300 text-xl tracking-tightest h-full child:leading-[56px]">
                                    <div>
                                        <Image
                                    ...
                                    <div className="group">
                                        ...
                                        <div className="hidden group-hover:flex flex-col absolute left-0 right-0 p-10 w-full shadow-normal transition-all child:tracking-normal child:transition-colors overflow-x-hidden delay-75 rounded-2xl text-zinc-700 dark:text-white text-base bg-white dark:bg-zinc-700">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                                                ...
                                                {categoryBlog?.map(attraction => (

                                                    <div className="flex flex-col" key={attraction._id}>
                                                        <h3 className="mb-4 text-xl">{attraction.title}</h3>
                                                        {attraction.children.map(child => (
                                                            <Link key={child._id} href={`/attractions/${attraction.slug}/${child.slug}`} className="hover:text-orange-400">{child.title}</Link>
                                                        ))}
